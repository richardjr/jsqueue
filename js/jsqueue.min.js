...
core.data={process_statment:function(b){for(var c,a=b,f=/([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s\#\*\-]*(\[.*?\])*[a-zA-Za.\_]*[\:]{0,1})/g;c=f.exec(b);)a=a.replace(c[1],'"'+core.data.uritodata(c[1])+'"');return a},htmlinject:function(b){var c,a=b;for(c in jsqueue.loops)for(var f=RegExp("~"+c+"~","g");f.exec(b);)a=a.replace("~"+c+"~",jsqueue.loops[c]);b=a;for(f=/\~([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s\#\*\-]*(\[.*?\])*[a-zA-Za.\_]*[\:]{0,1})/g;c=f.exec(b);){var d=c[1];c=c[1].replace(/\:$/,"");a=a.replace("~"+
d,this.uritodata(c))}f=/\@([a-zA-Z0-9\_]*\(.*\))/g;for(b=a;c=f.exec(b);)d=c[1],c=eval(c[1]),a=a.replace("@"+d,c);return a},uritodata:function(b,c){function a(a,b){var c=b.match(/^@(.*)/);return c?c[1]:(c=b.match(/^\*(.*)/))?JSON.stringify(a[c[1]]):void 0!==a?a[b]:""}function f(b){for(var c=b,f=/\[([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s\#\*\-]*)\]/g;d=f.exec(b);)c=c.replace("["+d[1]+"]","."+core.data.uritodata(d[1]));var d=c.match(/(.*?):\/\/(.*)/);switch(d[1]){case "global":return b=d[2].split(".").reduce(a,
window);case "jquery":return b=d[2].replace(/\//,""),b=$(b).val();case "reg":return b=d[2].replace(/\//,""),b=jsqueue.get_reg(b);case "request":return b=d[2].replace(/\//,""),core.data.getUrl(b);case "stack":return b=d[2].match(/(.*?)\/(.*)/),c=b[1].split(".").reduce(a,jsqueue.stack),b=b[2]?b[2].split(".").reduce(a,c):c,void 0===b?"":b;default:return"data uri ["+d[1]+"] is not valid"}}for(var d=b.split(","),e="",g=0;g<d.length;g++)if(void 0!==c&&!0===c)e+=f(d[g])+" ";else if(e=f(d[g]))break;return e},
check_params:function(b,c){var a=!0,f;for(f in b)c[f]||(console.error(b[f].message),a=!1);return a},serializer:function(b,c){return"object"===typeof c?"Object":c},datamunge:function(b){var c=this;$.each(b,function(a,f){c.datamunge_recursive(a,f,b,b,0)})},datamunge_recursive:function(b,c,a,f,d){var e=this;if(!(10<d))if(c instanceof Object)$.each(c,function(c,h){e.datamunge_recursive(c,h,a,f[b],d++)});else{var g;if("string"==typeof c&&(g=c.match(/\!jquery:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))){var h=g[1].replace(/:.*/,
"");jQuery(h).is("input:not(:checkbox),textarea,select")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):jQuery(h).is(":checkbox")?jQuery(h).is(":checked")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):f[b]="":f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).html())}if("string"==typeof c&&(g=c.match(/\!data:(.*)[:]{0,1}/))){var h=g[1].replace(/:.*/,""),k=function(a,b){return a[b]},l=h.split(".").reduce(k,a);c.match(/^\!data:(.*)[:]{0,1}$/)?
