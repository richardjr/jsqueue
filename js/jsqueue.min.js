...
core.data={htmlinject:function(b){var c,a=b,f;for(f in jsqueue.loops)for(var d=RegExp("~"+f+"~","g");d.exec(b);)a=a.replace("~"+f+"~",jsqueue.loops[f]);b=a;for(d=/\~([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s]*[\:]{0,1})/g;c=d.exec(b);)f=c[1],c=c[1].replace(/\:$/,""),a=a.replace("~"+f,this.uritodata(c));return a},uritodata:function(b){function c(a,b){var c=b.match(/^@(.*)/);return c?c[1]:a?a[b]:""}function a(a){for(var b=a,f=/\[([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s]*)\]/g;d=f.exec(a);)b=b.replace("["+d[1]+
"]","."+core.data.uritodata(d[1]));var d=b.match(/(.*?):\/\/(.*)/);switch(d[1]){case "global":return a=d[2].split(".").reduce(c,window);case "request":return console.log(d),b=d[1],a=(b=RegExp("[?&]"+encodeURIComponent(b)+"=([^&]*)").exec(location.search))?decodeURIComponent(b[1]):void 0,a;case "stack":return a=d[2].match(/(.*?)\/(.*)/),b=a[1].split(".").reduce(c,jsqueue.stack),a=a[2]?a[2].split(".").reduce(c,b):b,void 0===a?"":a;default:return"data uri ["+d[1]+"] is not valid"}}b=b.split(",");for(var f,
d=0;d<b.length&&!(f=a(b[d]));d++);return f},check_params:function(b,c){var a=!0,f;for(f in b)c[f]||(console.error(b[f].message),a=!1);return a},serializer:function(b,c){return"object"===typeof c?"Object":c},datamunge:function(b){var c=this;$.each(b,function(a,f){c.datamunge_recursive(a,f,b,b,0)})},datamunge_recursive:function(b,c,a,f,d){var e=this;if(!(10<d))if(c instanceof Object)$.each(c,function(c,h){e.datamunge_recursive(c,h,a,f[b],d++)});else{var g;if("string"==typeof c&&(g=c.match(/\!jquery:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))){var h=
g[1].replace(/:.*/,"");jQuery(h).is("input:not(:checkbox),textarea,select")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):jQuery(h).is(":checkbox")?jQuery(h).is(":checked")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):f[b]="":f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).html())}if("string"==typeof c&&(g=c.match(/\!data:(.*)[:]{0,1}/))){var h=g[1].replace(/:.*/,""),k=function(a,b){return a[b]},h=h.split(".").reduce(k,a);c.match(/^\!data:(.*)[:]{0,1}$/)?
f[b]=h:f[b]=c.replace(/\!data:.*[:]{0,1}/,h)}"string"==typeof c&&(g=c.match(/\!stack:\/\/(.*)[:]{0,1}/))&&(h=g[1].replace(/:.*/,""),g=h.split("/"),h=g[1],a=jsqueue.stack[g[0]],k=function(a,b){return a[b]},h=h.split(".").reduce(k,a),"[object Array]"===Object.prototype.toString.call(h)?f[b]=h:f[b]=c.replace(/\!stack:\/\/.*[:]{0,1}/,h));"string"==typeof c&&(g=c.match(/\!request:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))&&(h=g[1].replace(/:.*/,""),f[b]=c.replace(/\!request:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,function(a){if(a=
RegExp("[?&]"+encodeURIComponent(a)+"=([^&]*)").exec(location.search))return decodeURIComponent(a[1])}(h)))}}};
...
b(a).attr("id")+"][data-order=pre]").each(function(){var a=b(this).data();d.push(c.ng_workflow_exec(a))});d.push(c.ng_workflow_exec(g));b(".js-workflow-action[data-parent="+b(a).attr("id")+"][data-order!=pre]").each(function(){var a=b(this).data();a.chain&&"fail"==a.chain?e.push(c.ng_workflow_exec(a)):d.push(c.ng_workflow_exec(a))});b("wf-child",a).each(function(){var a=b(this).data();a.chain&&"fail"==a.chain?e.push(c.ng_workflow_exec(a)):d.push(c.ng_workflow_exec(a))});for(a=0;a<d.length;a++)null==
d[a]&&d.splice(a,1);a=d[0];d.splice(0,1);0<d.length&&(a.chain=d);0<e.length&&(a.fail_chain=e);jsqueue.add(a)},ng_workflow_exec:function(a){switch(a.action){case "post-form":return{component:"TOOLS",command:"TOOLS_REST_API",data:{uri:this.options.uri,form:a.form,validatefunction:a.validatefunction,json:a.data},datamode:a.mode?a.mode:!1};default:var c={};a.form?core.forms.encode(a.form,c):c=b.extend({},a.data,a.merge);c={component:a.component,command:a.command,data:c,datamode:a.mode?a.mode:!1,stackname:a.stackname?
a.stackname:!1};a.reg&&(c.reg=a.reg);return c}}};b.fn.jsWorkflow=function(a){return this.each(function(){var f=b(this),d=f.data("jsQueueDebug"),e="object"==typeof a&&a;d||f.data("jsWorkflow",d=new c(this,e));if("string"==typeof a)d[a]()})};b.fn.jsWorkflow.Constructor=c})(window.jQuery);
