function jsqueue_main(){this.construct=function(){this.pid=1;this.components={};this.debug=!1;this.loops={};this.queue=[];this.registers={};this.config={auto_start:"true",maxlife:5E4,timeout:10};this.stack={}};this["debugger"]=function(){console.log("jsqueue debugger enabled");return this.debug=!0};this.register=function(b,c,a,f){this.components[b]={mode:c||"plugin",aclass:a||"",afunction:f||"",state:"inactive"}};this.set_reg=function(b,c){this.registers[b]="object"===typeof c?$.extend({},c):c;this.debug&&
...
core.data={process_statment:function(b){for(var c,a=b,f=/([a-zA-Z]*:\/\/[a-zA-Z_\/\.\@\s]*)/g;c=f.exec(b);)a=a.replace(c[1],'"'+core.data.uritodata(c[1])+'"');return a},htmlinject:function(b){var c,a=b,f;for(f in jsqueue.loops)for(var d=RegExp("~"+f+"~","g");d.exec(b);)a=a.replace("~"+f+"~",jsqueue.loops[f]);b=a;for(d=/\~([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s]*(\[.*?\])*[a-zA-Za.\_]*[\:]{0,1})/g;c=d.exec(b);)f=c[1],c=c[1].replace(/\:$/,""),a=a.replace("~"+f,this.uritodata(c));return a},uritodata:function(b){function c(a,
b){var c=b.match(/^@(.*)/);return c?c[1]:a?a[b]:""}function a(a){for(var b=a,f=/\[([a-zA-Z\.]*:\/\/[a-zA-Z_\/\.0-9@\s]*)\]/g;d=f.exec(a);)b=b.replace("["+d[1]+"]","."+core.data.uritodata(d[1]));var d=b.match(/(.*?):\/\/(.*)/);switch(d[1]){case "global":return a=d[2].split(".").reduce(c,window);case "request":return a=d[2].replace(/\//,""),core.data.getUrl(a);case "stack":return a=d[2].match(/(.*?)\/(.*)/),b=a[1].split(".").reduce(c,jsqueue.stack),a=a[2]?a[2].split(".").reduce(c,b):b,void 0===a?"":
a;default:return"data uri ["+d[1]+"] is not valid"}}b=b.split(",");for(var f,d=0;d<b.length&&!(f=a(b[d]));d++);return f},check_params:function(b,c){var a=!0,f;for(f in b)c[f]||(console.error(b[f].message),a=!1);return a},serializer:function(b,c){return"object"===typeof c?"Object":c},datamunge:function(b){var c=this;$.each(b,function(a,f){c.datamunge_recursive(a,f,b,b,0)})},datamunge_recursive:function(b,c,a,f,d){var e=this;if(!(10<d))if(c instanceof Object)$.each(c,function(c,h){e.datamunge_recursive(c,
h,a,f[b],d++)});else{var g;if("string"==typeof c&&(g=c.match(/\!jquery:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))){var h=g[1].replace(/:.*/,"");jQuery(h).is("input:not(:checkbox),textarea,select")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):jQuery(h).is(":checkbox")?jQuery(h).is(":checked")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):f[b]="":f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).html())}if("string"==typeof c&&(g=c.match(/\!data:(.*)[:]{0,1}/))){var h=
g[1].replace(/:.*/,""),k=function(a,b){return a[b]},h=h.split(".").reduce(k,a);c.match(/^\!data:(.*)[:]{0,1}$/)?f[b]=h:f[b]=c.replace(/\!data:.*[:]{0,1}/,h)}"string"==typeof c&&(g=c.match(/\!stack:\/\/([#a-zA-Z\-_\.0-9\/]*)[:]{0,1}/))&&(h=g[1].replace(/:.*/,""),g=h.split("/"),h=g[1],a=jsqueue.stack[g[0]],k=function(a,b){return a[b]},h=h.split(".").reduce(k,a),"[object Array]"===Object.prototype.toString.call(h)?f[b]=h:f[b]=c.replace(/\!stack:\/\/[#a-zA-Z\-_\.0-9\/]*[:]{0,1}/,h));"string"==typeof c&&
(g=c.match(/\!request:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))&&(h=g[1].replace(/:.*/,""),f[b]=c.replace(/\!request:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,core.data.getUrl(h)))}},getUrl:function(b){return(b=RegExp("[?&]"+encodeURIComponent(b)+"=([^&]*)").exec(location.search))?decodeURIComponent(b[1]):!1}};
