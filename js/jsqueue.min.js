...
c&&(b(f).html(core.data.htmlinject(b(a.template).html())),b("wf-default",f).remove());b(this).remove()});a=b("wf-default",f);if(0<a.length){var e=a.data();b(f).html(core.data.htmlinject(b(e.template).html()));a.remove()}d(this);b(f).contents().unwrap()}else console.log(this)};document.registerElement("wf-switch",{prototype:g});g=Object.create(HTMLElement.prototype);g.attachedCallback=function(){var a=b(this).data();if(core.data.check_params({source:{message:"No source specified"},template:{message:"No template specified"}},
a)){var c=a.index||"index",f=core.data.uritodata(a.source),e=a.target||this;if(0>=b(e).length)console.error("Target does not exist:"+a.target);else{jsqueue.loops[c]=0;if("[object Array]"===Object.prototype.toString.call(f))for(var g=0;g<f.length;g++)jsqueue.loops[c]=g,a.stackname&&jsqueue.push_name(a.stackname,f[g]),b(e).append(core.data.htmlinject(b(a.template).html())),d(e);else if("object"===typeof f)for(g in f)jsqueue.loops[c]=g,a.stackname&&jsqueue.push_name(a.stackname,{key:g,value:f[g]}),b(e).append(core.data.htmlinject(b(a.template).html())),
