...
h){e.datamunge_recursive(c,h,a,f[b],d+1)});else{var g;if("string"==typeof c&&(g=c.match(/\!jquery:([#a-zA-Z\-_\.0-9]*)[:]{0,1}/))){var h=g[1].replace(/:.*/,"");jQuery(h).is("input:not(:checkbox),textarea,select")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):jQuery(h).is(":checkbox")?jQuery(h).is(":checked")?f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).val()):f[b]="":f[b]=c.replace(/\!jquery:[#a-zA-Z\-_\.0-9]*[:]{0,1}/,jQuery(h).html())}if("string"==typeof c&&
...
jsqueue.finished(a.PID)},TOOLS_UPDATE_HTML:function(a){b(a.element).html(a.value);jsqueue.finished(a.PID)},TOOLS_START_CAROUSEL:function(a){b(".carousel").carousel();jsqueue.finished(a.PID)},TOOLS_SET_ATTRIBUTE:function(a){b(a.target).attr(a.attributes);jsqueue.finished(a.PID)},TOOLS_RELOAD:function(a){window.location.reload();jsqueue.finished(a.PID)},TOOLS_SLIDE_TOGGLE:function(a){var c=a.offset||20;if(a.target){var d=b(a.target);0>d.position().left?(d.css({left:"0px"}),d.addClass("active")):(d.css({left:"-"+
(d.width()-c)+"px"}),d.removeClass("active"))}else console.warn("TOOL_SLIDE_TOGGLE called with no target");jsqueue.finished(a.PID)},TOOLS_FORMAT_DATA:function(a){function b(a,c){return a[c]}var c={},e;for(e in a.paths)if("root"==a.paths[e])c[e]=a;else{var g=e.split(".");1==g.length?c[e]=a.paths[e].split(".").reduce(b,a):(c[g[0]]={},c[g[0]][g[1]]=a.paths[e].split(".").reduce(b,a))}jsqueue.push(a.PID,c);jsqueue.finished(a.PID)},TOOLS_SET_REG:function(a){function b(a,c){return a[c]}if(a.path){var c=
...
typeof a.triggers[c].afunction)b(a.triggers[c].aclass).on(a.triggers[c].atrigger,window[a.triggers[c].afunction]);else b(a.triggers[c].aclass).on(a.triggers[c].atrigger,a.triggers[c].afunction);a.global&&a.global();jsqueue.finished(a.PID)},helper_replace_value:function(a,c,d,e){var g=this;if(c instanceof Object)b.each(c,function(b,c){g.helper_replace_value(b,c,d,e[a])});else{var h;if("string"==typeof c&&(h=c.match(/%(.*?)%/))){console.info("TOOLS_REST_API call is using depricated helper_replace_value % value. Please move to !data: format");
