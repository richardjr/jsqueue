...
c)};this.delete_name_element=function(b,c){function a(b,c){if("string"==typeof c)return a(b,c.split("."));if(1==c.length)delete b[c[0]];else return a(b[c[0]],c.slice(1))}a(this.stack[b],c)};this.delete_name_array_element=function(b,c,a){c.split(".").reduce(function(a,b){return a[b]},this.stack[b]).splice(a,1)};this.set_name_value=function(b,c,a){function f(a,b,c){return"string"==typeof b?f(a,b.split("."),c):1==b.length&&void 0!==c?a[b[0]]=c:0==b.length?a:f(a[b[0]],b.slice(1),c)}f(this.stack[b],c,
...
break}};this.finished=function(b){for(var c=0;c<this.queue.length;c++)if(this.queue[c].data.PID==b)if("running"==this.queue[c].state){this.debug&&console.log(this.queue[c].data.PID+" Running process reports finished");this.queue[c].state="finished";break}else if("triggered"==this.queue[c].state){this.queue[c].logic?(b=this.queue[c].chain[0],b.stack=this.queue[c].stack,this.queue[c].chain.splice(0,1),0<this.queue[c].chain.length&&(b.chain=this.queue[c].chain),this.queue[c].fail_chain&&0<this.queue[c].fail_chain.length&&
(b.fail_chain=this.queue[c].fail_chain),this.queue[c].state="finished",this.debug&&console.log(this.queue[c].data.PID+" triggered process with logic true reports finished"),this.add(b)):(this.queue[c].state="finished",this.queue[c].fail_chain&&(b=this.queue[c].fail_chain[0],b.stack=this.queue[c].stack,this.queue[c].fail_chain.splice(0,1),0<this.queue[c].fail_chain.length&&(b.chain=this.queue[c].fail_chain),this.debug&&console.log(this.queue[c].data.PID+" triggered process with logic false reports finished"),
this.add(b)));break}};this.activate=function(b,c){this.components[b].state="active";c&&(this.components[b].object=c);this.debug&&console.warn("Component "+b+" Reports Active")};this.construct()}var jsqueue=null;
...
c&&(b(e).html(core.data.htmlinject(b(a.template).html())),b("wf-default",e).remove());b(this).remove()});a=b("wf-default",e);if(0<a.length){var f=a.data();b(e).html(core.data.htmlinject(b(f.template).html()));a.remove()}d(this);b(e).contents().unwrap()}else console.log(this)};document.registerElement("wf-switch",{prototype:g});g=Object.create(HTMLElement.prototype);g.attachedCallback=function(){var a=b(this).data();if(core.data.check_params({source:{message:"No source specified"},template:{message:"No template specified"}},
