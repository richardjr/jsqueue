...
c){return"object"===typeof c?"Object":c},datamunge:function(b){var c=this;$.each(b,function(a,d){c.datamunge_recursive(a,d,b,b,0)})},datamunge_recursive:function(b,c,a,d,e){var f=this;if(!(10<e))if(c instanceof Object)$.each(c,function(c,h){f.datamunge_recursive(c,h,a,d[b],e++)});else{var g;if("string"==typeof c&&(g=c.match(/\!jquery:(.*)[:]{0,1}/))){var h=g[1].replace(/:.*/,"");jQuery(h).is("input:not(:checkbox),textarea,select")?d[b]=c.replace(/\!jquery:.*[:]{0,1}/,jQuery(h).val()):jQuery(h).is(":checkbox")?
jQuery(h).is(":checked")?d[b]=c.replace(/\!jquery:.*[:]{0,1}/,jQuery(h).val()):d[b]="":d[b]=c.replace(/\!jquery:.*[:]{0,1}/,jQuery(h).text())}if("string"==typeof c&&(g=c.match(/\!data:(.*)[:]{0,1}/))){var h=g[1].replace(/:.*/,""),k=function(a,b){return a[b]},h=h.split(".").reduce(k,a);c.match(/^\!data:(.*)[:]{0,1}$/)?d[b]=h:d[b]=c.replace(/\!data:.*[:]{0,1}/,h)}"string"==typeof c&&(g=c.match(/\!stack:\/\/(.*)[:]{0,1}/))&&(h=g[1].replace(/:.*/,""),g=h.split("/"),h=g[1],a=jsqueue.stack[g[0]],k=function(a,
b){return a[b]},h=h.split(".").reduce(k,a),d[b]=c.replace(/\!stack:\/\/.*[:]{0,1}/,h))}}};
...
a.name+'" style="width: 0%;"></div></div>');jsqueue.finished(a.PID)},TOOLS_PROGRESS_UPDATE:function(a){a.element&&a.progress&&b(a.element).css("width",a.progress+"%");jsqueue.finished(a.PID)},TOOLS_RUN_JAVASCRIPT:function(a){eval(a["function"]+"("+a.args+");")},TOOLS_REST_API:function(a){var d=this,c={},f=b.extend(!0,{},a);f.uri||(f.uri=d.options.uri);if(f.form){if(f.validatefunction&&!window[f.validatefunction](f.form))return;core.forms.encode(f.form,c)}f.json&&b.each(a.json,function(a,b){d.helper_replace_value(a,
b,f,f.json)});c=b.extend(!0,{},f.json,c);c=encodeURIComponent(JSON.stringify(c));-1!=navigator.appVersion.indexOf("MSIE 9")||-1!=navigator.appVersion.indexOf("MSIE 8")?b.ajax({type:"POST",url:f.uri,data:c,async:!0,processData:!1,traditional:!1,headers:a.headers||{},success:function(b){a.nostack||(f.json&&f.json.ignoredata?jsqueue.push(f.PID,b.data):jsqueue.push(f.PID,b));jsqueue.finished(f.PID);d.debug&&(console.info("jsTools->call_api"),console.info(b));f.success&&f.success(b)},error:function(a){d.debug&&
(console.warn("jsTools->call_api"),console.warn(a))}}):b.ajax({type:"POST",url:f.uri,data:c,async:!0,crossDomain:!0,xhrFields:{withCredentials:!0},processData:!1,traditional:!1,headers:a.headers||{},success:function(b){a.nostack||(f.json&&f.json.ignoredata?jsqueue.push(f.PID,b.data):jsqueue.push(f.PID,b));jsqueue.finished(f.PID);d.debug&&(console.info("jsTools->call_api"),console.info(b));f.success&&f.success(b)},error:function(a){d.debug&&(console.warn("jsTools->call_api"),console.warn(a))}})}};
b.fn.jsTools=function(a){return this.each(function(){var d=b(this),e=d.data("jsQueueDebug"),f="object"==typeof a&&a;e||d.data("jsTools",e=new c(this,f));if("string"==typeof a)e[a]()})};b.fn.jsTools.Constructor=c})(window.jQuery);(function(b){function c(a,c){function e(a){document.dispatchEvent(new CustomEvent("readystatechange"))}var f=this;f.options=c;f.div=a;b(a).on({command:function(a,b,c){f[b](c)}});jsqueue.activate("WORKFLOW");!0==f.options.auto&&f.WORKFLOW_START({});var g=Object.create(HTMLElement.prototype);g.attachedCallback=function(){b(this).data();f.ng_workflow_build(this);b(this).remove()};document.registerElement("wf-load",{prototype:g});g=Object.create(HTMLElement.prototype);g.createdCallback=function(){b(this).data();
...
template:{message:"No template specified"}},a)){var c=a.index||"index",d=core.data.uritodata(a.source);jsqueue.loops[c]=0;if("[object Array]"===Object.prototype.toString.call(d))for(var f=0;f<d.length;f++){jsqueue.loops[c]=f;a.stackname&&jsqueue.push_name(a.stackname,d[f]);var g=a.target||this;b(g).append(core.data.htmlinject(b(a.template).html()));e(g)}else for(f in d)jsqueue.loops[c]=f,a.stackname&&jsqueue.push_name(a.stackname,{key:f,value:d[f]}),g=a.target||this,b(g).append(core.data.htmlinject(b(a.template).html())),
...
e.push(c.ng_workflow_exec(a))});e.push(c.ng_workflow_exec(g));b(".js-workflow-action[data-parent="+b(a).attr("id")+"][data-order!=pre]").each(function(){var a=b(this).data();a.chain&&"fail"==a.chain?f.push(c.ng_workflow_exec(a)):e.push(c.ng_workflow_exec(a))});b("wf-child",a).each(function(){var a=b(this).data();a.chain&&"fail"==a.chain?f.push(c.ng_workflow_exec(a)):e.push(c.ng_workflow_exec(a))});for(a=0;a<e.length;a++)null==e[a]&&e.splice(a,1);a=e[0];e.splice(0,1);0<e.length&&(a.chain=e);0<f.length&&
...
function(a){return this.each(function(){var d=b(this),e=d.data("jsQueueDebug"),f="object"==typeof a&&a;e||d.data("jsWorkflow",e=new c(this,f));if("string"==typeof a)e[a]()})};b.fn.jsWorkflow.Constructor=c})(window.jQuery);
